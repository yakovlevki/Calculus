...
\begin{comm}
    Пусть $T = \{\Delta_j\}$
    \[\Omega(f, T) = \sum \limits_i \left(\ \sup \limits_{x \in \Delta_i} f(x) - \inf \limits_{x \in \Delta_i} f(x)\right)\cdot |\Delta_i|=\sum\limits_{i}\omega_i\cdot |\Delta_i|\]
    $\omega_i:=\sup \limits_{x \in \Delta_i} f(x) - \inf \limits_{x \in \Delta_i} f(x)$ называют колебаниями $f$ на $\Delta_i$.
\end{comm}
\noindent В одномерном случае доказывалось утверждение
\setcounter{numtheorem}{-1}
\begin{numtheorem}[Критерий Дарбу интегрируемости на отрезке]
...
\begin{proof} Докажем по схеме $(1)\Rightarrow(2) \Rightarrow(4) \Rightarrow (3) \Rightarrow (1)$
    \begin{enumerate}
        \item $(1) \Rightarrow (2)$:
            ...
            %\[|I - \overline{\overline{s}}(f, T)| < \epsilon,\ \ |I - \underline{\underline{S}}(f, T)|<\epsilon\]
        ...
        \item $(2)\Rightarrow(4)$: Пусть
            \[\inf \limits_{T} \underline{\underline{S}}(f, T) = \sup \limits_{T} \overline{\overline{s}}(f, T)\]
        ...
        \item $(4) \Rightarrow (3)$: Пусть $T$ --- произвольное разбиение, такое, что $d(T)<\delta$. По пункту 4:
            \[\forall \epsilon>0\ \exists\ T_{\epsilon}:\ \Omega(f,T_{\epsilon})<\frac{\epsilon}{2}\]
            Пусть разбиение $T_1$ получено объединением сеток разбиений $T_{\epsilon}$ и $T$, причем добавилось $p$ разрезов. Тогда по свойству $\ref{razrezy}$:
            \[0\leq \underline{\underline{S}}(f,T)-\underline{\underline{S}}(f,T_1)\leq (M-m)\cdot d^{k-1}\cdot d(T)\cdot p\]
            \[0\leq \overline{\overline{s}}(f,T_1)-\overline{\overline{s}}(f,T)\leq (M-m)\cdot d^{k-1}\cdot d(T)\cdot p\]
...
\begin{proof}
    ...
    (f\in \mathcal{UC}(f))$:
    \[\forall \epsilon>0\ \exists\ \delta>0,\ \forall x_1,x_2,\ \rho(x_1,x_2)<\delta: |f(x_1)-f(x_2)|<\frac{\epsilon}{|\Pi|} \eqno{(*)}\]
    Хотим показать, что выполнен критерий Дарбу:
    \[\Omega(f,T)=\sum\limits_{i}\omega_i\cdot |\Delta_i|<\epsilon\]
    При этом из $(*)$ имеем
    \[\omega_i=\sup\limits_{x_1\in \Delta_i}f(x_1)-\inf\limits_{x_2\in \Delta_i}f(x_2)\leq \frac{\epsilon}{|\Pi|}\]
    таким образом
    \[\Omega(f,T)\leq \frac{\epsilon}{|\Pi|}\sum\limits_{i}|\Delta_i|=\frac{\epsilon}{|\Pi|}\cdot |\Pi|=\epsilon\]
\end{proof}
\newpage