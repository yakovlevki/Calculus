...
\end{theorem}
\begin{proof}
    \textit{Приведем адаптированный вариант док-ва из \cite{podolsky}}\\
    ...
    \[\mu(\Phi_i) \leqslant \iint\limits_{K_i} |J_{\phi^{-1}}|\ d\hat{x}d\hat{y}\] \[\iint\limits_{K_i} |J_{\phi^{-1}}|\ d\hat{x}d\hat{y} \leqslant M_i \mu(K_i), \ M_i = \max\limits_{K_i}(|J_{\phi^{-1}}|) = \max\limits_{K_i}(|J_{\phi}|^{-1}) = (\min\limits_{\Phi_i}(|J_{\phi}|))^{-1}\]
    то есть если $m_i = \min\limits_{\Phi_i}(|J_{\phi}|)$, то $M_i = \frac{1}{m_i} \Longrightarrow \mu(\Phi_i) \leqslant \frac{1}{m_i}\mu(K_i)$.\\
