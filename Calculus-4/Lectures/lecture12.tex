...
\end{proof}
\begin{example} [Вычисление интеграла Эйлера Пуассона с помощью кратного интеграла] $\\$
    Вычислим интеграл
    \[I=\int\limits_{-\infty}^{+\infty}e^{-x^2}\ dx\]
    Рассмотрим двойной интеграл
    \begin{multline*}
        \iint\limits_{\R^2}e^{-(x^2+y^2)}\ dxdy=\int\limits_{-\infty}^{+\infty}\left(\ \int\limits_{-\infty}^{+\infty}e^{-(x^2+y^2)}\ dy\right)dx=\\
        =\left(\ \int\limits_{-\infty}^{+\infty} e^{-y^2}\ dy\right)\cdot \left(\ \int\limits_{-\infty}^{+\infty}e^{-x^2}\ dx\right)=I^2
    \end{multline*}
    Перейдем к полярным координатам $(r,\phi)$ в двойном интеграле. При этом переходе $\det{J}=r$. Значит
    \[\iint\limits_{\R^2}e^{-(x^2+y^2)}\ dxdy=\int\limits_{0}^{2\pi}\left(\int\limits_{0}^{+\infty}r\cdot e^{-r^2}\ dr\right)d\phi=2\pi\cdot \int\limits_{0}^{+\infty}r\cdot e^{-r^2}\ dr\]
    Сделаем замену $r^2=u$ при такой замене $dr=\frac{du}{2r}$ и
    \[2\pi\cdot \int\limits_{0}^{+\infty}r\cdot e^{-r^2}\ dr=2\pi\cdot \frac{1}{2}\cdot \int\limits_{0}^{=\infty}e^{-u}\ du=2\pi\cdot \frac{1}{2}=\pi\]
