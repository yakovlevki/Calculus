\section{Лекция 14}
\begin{theorem}[Сферические ккординаты в многомерном пространстве] $\\$
...
\begin{proof}
    ...
    поскольку оба интграла сущетсвуют, то, устремив $n\to \infty$, получим утверждение теоремы.
\end{proof}
\begin{reminder}
...
\begin{theorem}[Критерий Коши существования несобственного кратного интграла] $\\$
    Пусть $\Omega$ --- открытое множество, $f\in \mathcal{R}_{\text{loc}}(\Omega)$. Тогда $f\in \widetilde{\mathcal{R}}(\Omega)$ тогда и только тогда, когда $\forall \epsilon>0\ \exists\ K_{\epsilon}\subset \Omega$ --- измеримый компакт такой, что $\forall K\subset\Omega\setminus K_{\epsilon}$:
...
\begin{proof}\tab
    \begin{itemize}
        ...
        она имеет предел, а значит она функдаментальна, то есть \\
        ...
        Поскольку $K_1$ --- компакт и $f\in \mathcal{R_{\text{loc}}}$, то 
...
% \begin{example}
%     Рассмотрим функцию $f(x_1,\dots,x_k)=\frac{1}{(\sqrt{x_1^2+\dots+x_k^2})^p}$ и интеграл
%     \[\idotsint\limits_{0<\sqrt{x_1^2+\dots+x_k}<1}f(x_1,\dots,x_k)\ dx_1 \dots dx_k\]
%     Поймем при каких $p$ он сходится: Возьмем $K_n=\frac{1}{n}\leq \sqrt{x_1^2+\dots+x_k^2}\leq 1-\frac{1}{n}$
%     \begin{multline*}
%         \idotsint\limits_{K_n}f(x_1,\dots,x_k)\ dx_1 \dots dx_k=\\
%         =\idotsint\limits_{\frac{1}{n}\leq r\leq 1-\frac{1}{n}}f(\phi(x_1,\dots,x_k))\cdot r^{k-1}\cdot \cos{\alpha_2}\cdot \dots \cos^{k-2}{\alpha_{k-1}}\ dr d\alpha_1 \dots d\alpha_{k-1}=\\
%         =\int\limits_{\frac{1}{n}}^{1-\frac{1}{n}}\frac{1}{r^{p-k+1}}dr\cdot \int\limits_{0}^{2\pi}1 d\alpha_1\cdot \int\limits_{-\frac{\pi}{2}}^{\frac{\pi}{2}}\cos{\alpha_{2}}d\alpha_2 \dots \int\limits_{-\frac{\pi}{2}}^{\frac{\pi}{2}}\cos{\alpha_{k-1}}d\alpha_{k-1}=\\
%         =\frac{1}{k}\cdot2\pi\cdot B(\frac{1+1}{2},\frac{0+1}{2})\cdot B(\frac{2+1}{2},\frac{0+1}{2})\dots B(\frac{k-1}{2},\frac{0+1}{2})=\\
%         =\frac{1}{k}\cdot\frac{\Gamma(1)\Gamma(\frac{1}{2})}{\Gamma(\frac{3}{2})}\cdot \frac{\Gamma(\frac{3}{2})\Gamma(\frac{1}{2})}{\Gamma(2)} \dots \frac{\Gamma(\frac{k-1}{2})\Gamma(\frac{1}{2})}{\Gamma(\frac{k}{2})}=\\
%         =\frac{2\pi(\sqrt{\pi})^{k-2}}{k\cdot \Gamma(\frac{k}{2})}=\frac{2\cdot \pi^{\frac{k}{2}}}{k\cdot \Gamma(\frac{k}{2})}
%     \end{multline*}
%     тут поправить
% \end{example}
\newpage