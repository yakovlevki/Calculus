...
\begin{proof}
    ...
    % \[\forall \epsilon>0\ \exists\ T_1,T_2,\ \partial \Phi_1\subset T_1,\ \partial\Phi_2\subset T_2: \mu(T_1)<\frac{\epsilon}{2},\ \mu(T_2)<\frac{\epsilon}{2}\]
    % Тогда $P_1=T_1\cup Q_1,\ P_2=T_2\cup Q_2$, где $Q_1\subset \Phi_1,\ Q_2\subset \Phi_2$. Таким образом
    % \[\mu(\Phi_1)+\mu(\Phi_2)\leq \mu(P_1)+\mu(P_2)\leq\mu(Q_1)+\mu(Q_2)+\mu(T_1)+\mu(T_2)\leq \mu(\Phi_1\sqcup\Phi_2)+\epsilon\]
    % Значит 
    % \[\mu(\Phi_1\sqcup\Phi_2)\geq \mu(\Phi_1)+\mu(\Phi_2)\]
\end{proof}
\begin{statement}
