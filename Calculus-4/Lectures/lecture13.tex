...
\[f^+(x)=\frac{f(x)+|f(x)|}{2},\ \ f^-(x)=\frac{|f(x)|-f(x)}{2}\]
...
\begin{lemma}
    ...
    причем на $F_{\epsilon}$ выполнено, что $f\equiv f^+$.
\end{lemma}
\begin{proof}
    Пусть $T=\{\Delta_j\}$ --- разбиение $\Omega$.
    \[\overline{\overline{s}}(f^+, T)=\sum\limits_{j} \inf\limits_{\Delta_j}f^+\cdot \mu(\Delta_j)\]
    Заметим, что если $\exists\ x\in \Delta_j$, что $f(x)\leq 0$, то $f^+(x)=0$, а значит $\inf\limits_{\Delta_j}f^+=0$. Поэтому достаточно суммировать только по тем $\Delta_j$, на которых принимаюстя положительные значения. Обозначим такие множества $\Delta_j'$. Тогда
    ...
    Поскольку 
    \[\idotsint\limits_{K}f^+ dx_1 \dots dx_k=\sup\limits_T \overline{\overline{s}}(f^+, T)\] 
    то по свойству супремума $\forall \epsilon>0\ \exists\ T$:
    \[\overline{\overline{s}}(f^+, T)>\idotsint\limits_{K}f^+\ dx_1 \dots dx_k -\epsilon\]
    Обозначим
    \[F_{\epsilon}=\bigcup\limits_{j} \Delta_j'\] %по положительным
    Пусть $T'$ --- разбиение $F_{\epsilon}$, состоящее из множеств $\Delta_j'$. Тогда
    ...
    Отсюда
    \[\idotsint\limits_{F_{\epsilon}}f\ dx_1\dots dx_k\geq \overline{\overline{s}}(f,T')= \overline{\overline{s}}(f^+,T)> \idotsint\limits_{K}f^+\ dx_1 \dots dx_k -\epsilon\]
\end{proof}
\begin{theorem}[Критерий интегрируемости в несобственном смысле] \label{kritintnes} $\\$
...
\begin{proof}\tab % По Солодову
    \begin{itemize}
        ...
        \item[$(\Rightarrow)$:] Предположим, что $f\in \widetilde{\mathcal{R}}(\Omega)$, но $|f|\not\in \widetilde{\mathcal{R}}(\Omega)$. Это в точности означает, что
        ...
        \[\lim\limits_{m\to \infty}\idotsint\limits_{K'_m}f\ dx_1 \dots dx_k\]
        то есть $f\not\in \widetilde{\mathcal{R}}(\Omega)$, противоречие.
    \end{itemize}
\end{proof}
\newpage