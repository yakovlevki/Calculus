...
\begin{comm}
    Множество $\Phi$ измеримо по Жордану $\Leftrightarrow \overline{\Phi}$ измеримо по Жордану, причем $\mu(\Phi)=\mu(\overline{\Phi})$. 
    %Однако аналогичное утверждение неверно для понятия измеримости по Лебегу:
    %\[\nu(\Q_{[0,1]})=0,\ \ \nu(\overline{\Q_{[0,1]}})=1\] 
\end{comm}
\noindent Далее пусть $\Phi$ --- измеримое по Жордану множество.
\begin{definition}
    Пусть $f$ определена на $\Phi$, $\Pi$ --- произвольный брус, содержащий $\Phi$. Тогда
    \[\idotsint\limits_{\Phi} f(x_1,\dots,x_k)\ dx_1\dots dx_k=\idotsint\limits_{\Pi} f(x_1,\dots,x_k)\chi_{\Phi}(x_1,\dots,x_k)\ dx_1\dots dx_k\]
\end{definition}
\begin{statement}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \[\forall \epsilon>0\ \exists\ T_1: \Omega(\widehat{f}, T_1)<\frac{\epsilon}{2}\]
        Возьмем брус $\Pi_1'$ такой, что $\Pi_1\subset \Pi_1'\subset \Pi_2$, причем
        \[\mu(\Pi_1'\setminus \Pi_1)=\mu(\Pi_1')-\mu(\Pi_1)<\frac{\epsilon}{4M}\]
        где $M=\sup |\widehat{f}|$. Рассмотрим разбиение $T_2$ бруса $\Pi_2$ такое, что оно совпадает с $T_1$ на $\Pi_1$ и содержит разрезы по граням $\Pi_1'$. Тогда брусы разбиения $T_2$ можно разбить на три группы:
        \begin{enumerate}
...
\begin{proof}
    Без доказательства.
\end{proof}
\begin{theorem} [Достаточное условие интегрируемости на $\Phi$] \label{dostyslov} $\\$
...
\begin{proof}
    ...
    значит 
    \[\Omega(\widehat{f}, T)<\frac{\epsilon}{2}+\frac{\epsilon}{2}=\epsilon\]
\end{proof}
\begin{consequense}
...
\begin{proof}
    ...
    Следовательно мера Жордана множества точек разрыва равна нулю, а значит $f\in \mathcal{R}(\Phi)$. В любом брусе найдется точка, в которой $f(x)=0$, иначе этот брус лежит в $S$, а тогда $\mu(S)>0$ и получаем противоречие. Значит для каждого разбиения существует разметка, такая, что $\sigma(f, T, H)=0$, а значит 
    \[\idotsint\limits_{\Phi}f(x_1,\dots,x_k)\ dx_1\dots dx_k=\sigma(f, T, H)=0\]
\end{proof}
