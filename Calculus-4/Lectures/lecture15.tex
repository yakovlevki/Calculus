...
\begin{definition}\tab
    \begin{enumerate}
        ...
        \item Пусть $\phi_1:[\alpha_1, \beta_1]\to \R^k,\ \phi_2:[\alpha_2, \beta_2]\to \R^k$. Считаем, что $\phi_1$ и $\phi_2$ эквивалентны (параметризуют одну и ту же кривую), если существует строго монотонная биекция $g: [\alpha_1, \beta_1]\to [\alpha_2, \beta_2]$ такая, что $\phi_1\equiv \phi_2 \circ g$ на $[\alpha_1, \beta_1]$.
        \item Кривой называется класс эквивалентности параметризаций по указанному отношению.
    \end{enumerate}
\end{definition}
\begin{definition}\tab
    \begin{enumerate}
        \item Пусть $\phi_1:[\alpha_1, \beta_1]\to \R^k,\ \phi_2:[\alpha_2, \beta_2]\to \R^k$. Считаем, что $\phi_1$ и $\phi_2$ эквивалентны (параметризуют одну и ту же кривую), если существует строго возрастающая биекция $g: [\alpha_1, \beta_1]\to [\alpha_2, \beta_2]$ такая, что $\phi_1\equiv \phi_2f \circ g$ на $[\alpha_1, \beta_1]$.
        \item Ориентированной кривой называется класс эквивалентности параметризаций по указанному отношению.
    \end{enumerate}
\end{definition}
\begin{definition}\tab
    \begin{enumerate}
...
\begin{reminder}\tab
    \begin{enumerate}
        \item Функция $f:[\alpha,\beta]\to \R$ имеет ограниченную вариацию, если
        \[\var(f,[\alpha,\beta])=\sup\limits_{T} \sum\limits_{i=1}^{n}|f(t_i)-f(t_{i-1})|<+\infty\]
        $T$ --- разбиение отрезка $[\alpha,\beta]$.
        \item Отображение $f:[\alpha,\beta]\to \R^k$ имеет ограниченную вариацию, если
        \[\sum\limits_{j=1}^{k}\var(x_j,[\alpha,\beta])<+\infty\]
...
\begin{proof}\tab
    \begin{enumerate}
        ...
        кривой $\gamma$, $T$ --- разбиение отрезка $[\alpha,\beta]$. Тогда
        \begin{multline*}
            \sum\limits_{i=1}^{n}\|\phi(t_i)-\phi(t_{i-1})\|=\\
            =\sum\limits_{i=1}^{n}\sqrt{(x_1(t_1)-x_1(t_{i-1}))^2+\dots+(x_k(t_1)-x_k(t_{i-1}))^2}\geq \tab[2cm]\\
            \geq \sum\limits_{i=1}^{n}|x_j(t_i)-x_j(t_{i-1})|
        \end{multline*}
        Для всех $j=1,\dots,k$. Перейдем к супремуму и получим, что
        \[\var(x_j, [\alpha,\beta])\leq |\gamma|\]
        Отсюда 
        \[\sum\limits_{j=1}^{k}\var(x_j, [\alpha,\beta])\leq k\cdot |\gamma|\] 
        ...
        \begin{multline*}
            \sum\limits_{i=1}^{n}\|\phi(t_i)-\phi(t_{i-1})\|=\\
            =\sum\limits_{i=1}^{n}\sqrt{(x_1(t_i)-x_1(t_{i-1}))^2+\dots+(x_k(t_i)-x_k(t_{i-1}))^2}\leq\tab[3cm]\\
            \tab[2cm]\sum\limits_{i=1}^{n}\sum\limits_{j=1}^{k}|x_j(t_i)-x_j(t_{i-1})|=\sum\limits_{j=1}^{k}\sum\limits_{i=1}^{n}|x_j(t_i)-x_j(t_{i-1})|
        \end{multline*}
        Перейдем к супремуму и получим, что
        \[|\gamma|\leq \sum\limits_{j=1}^{k}\var(x_j, [\alpha,\beta])\]
    \end{enumerate}
\end{proof}
\begin{example} [Неспрямляемой кривой] $\\$
    ...
    %тут будет картинка.
\end{example}
\begin{example}[Кривая Пеано] $\\$
    Рассмотрим квадрат $[0,1]^2$. Разделим его на 4 равных квадрата: (1) --- левый нижний, (2) --- левый верхний, (3) --- правый верхний, (4) --- правый нижний. Соединим соседние центры кадратов отрезками по правилу 
    \[(1)\to(2)\to(3)\to(4)\]
    Каждый из 4 квадратов делим ещё на 4. В каждом крупном квадрате повторяем тот же порядок обхода, но с поворотом или отражением так, чтобы ломаная непрерывно переходила из одного крупного квадрата в соседний. Первые три иттерации выглядят так:
    \[\includegraphics[width=0.8\textwidth]{Images/PeanoCurve.png}\]
...
\begin{definition}[Альтернативное определение криволинейного интеграла первого рода]\tab
    \begin{enumerate}
        ...
        \item Наконец, если
        \[\forall \epsilon>0\ \exists\ \delta>0: \forall T,\ d(\Lambda(T))<\delta: |\sigma(f,\Lambda(T), H)-I|<\epsilon\]
        то 
...
\end{proof}
\begin{theorem}
    Два определения криволинейных интегралов первого рода эквивалентны.
\end{theorem}
