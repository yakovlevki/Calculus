...
\begin{definition}\tab
    \begin{enumerate}
        \item Пусть $\phi_1:[\alpha_1, \beta_1]\to \R^k,\ \phi_2:[\alpha_2, \beta_2]\to \R^k$. Считаем, что $\phi_1$ и $\phi_2$ эквивалентны (параметризуют одну и ту же кривую), если существует строго возрастающая биекция $g: [\alpha_1, \beta_1]\to [\alpha_2, \beta_2]$ такая, что $\phi_1\equiv \phi_2 \circ g$ на $[\alpha_1, \beta_1]$.
...
\begin{proof}\tab
    \begin{enumerate}
        ...
        \begin{multline*}
            ...
            =\sum\limits_{i=1}^{n}\sqrt{(x_1(t_i)-x_1(t_{i-1}))^2+\dots+(x_k(t_i)-x_k(t_{i-1}))^2}\geq \tab[2cm]\\
