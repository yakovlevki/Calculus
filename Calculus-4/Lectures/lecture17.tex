...
\begin{definition}\tab
    \begin{enumerate}
        ...
        \item Векторное поле называется непрерывным (дифференцируемым), если непрерывна (дифференцируема) каждая $f_i$.
    \end{enumerate}
\end{definition}
\begin{definition}\tab
    \begin{enumerate}
...
\begin{proof}\tab
    \begin{enumerate}
        \item $(1)\Rightarrow(2)$: Пусть существует потенциал $U$, введем $A_1,\dots,A_n$ --- точки излома ломаной $\Lambda$. Обозначим $\Delta_j=[A_{j-1}, A_{j}]$ --- произвольное звено ломаной длины $l$. Обозначим $A_{j-1}=(x_1^{j-1},\dots,x_k^{j-1}),\ A_{j}=(x_1^{j},\dots,x_k^{j})$ и возьмем натуральную параметризацию для $\Delta_j$:
        \[\phi(t): \begin{cases}
            x_1=x_1^{j-1}+u_1 t,\\
            \tab[1.4cm]\vdots\\
            x_k=x_k^{j-1}+u_k t
        ...
        где $\Lambda_1$ --- первая ломаная от $A$ до $B$, а $\Lambda_2$ --- вторая ломаная от $A$ до $B$, а $\Lambda_2^-$ --- ломаная $\Lambda_2$, пройденная в обратном направлении. Тогда 
        \begin{multline*}
        ...
        \begin{multline*}
            \sigma(f,T_n,H)=\sum\limits_{j=1}^{n}\left(\sum\limits_{i=1}^{k}f_i(\phi(\xi_j))\Delta_{ij}\right)=\sum\limits_{j=1}^{n}\left(\sum\limits_{i=1}^{k}\int\limits_{\Delta_j}f_i(\phi(\xi_j))dx_i\right)=\\
            =\sum\limits_{j=1}^{n}\int\limits_{\Delta_j}f_1(\phi(\xi_j))\ dx_1+\dots+f_k(\phi(\xi_i))\ dx_k
        \end{multline*}
        ...
        \begin{multline*}
            \int\limits_{\Lambda(T_n)}f_1\ dx_1+\dots+f_k\ dx_k-\sigma(f, T_n, H)=\\
...
% \end{consequense}
\subsection{Формула Грина для областей специального вида}
\begin{lemma} $\\$
    Пусть $\gamma$ --- кривая, ограничивающая область $G$ в $\R^2,\ P\in \mathcal{C}^1(\overline{\Omega}),\\
    \phi_1,\phi_2\in \mathcal{C}([a,b])$. Тогда
...
\end{tikzpicture}
    \]
\end{lemma}
\begin{proof} Заметим, что $x=\text{const}$ на кривых $\gamma_2$ и $\gamma_4$ и напишем параметризации для $\gamma_1$ и $\gamma_3$:
    \[\gamma_1: \begin{cases}
        x=x\in [a,b],\\
        y=\phi_1(x)
...
\end{proof}
\begin{lemma}
    Пусть $\gamma$ --- кривая, ограничивающая область $G$ в $\R^2,\ Q\in \mathcal{C}^1(\overline{\Omega}),\\
    \phi_1,\phi_2\in \mathcal{C}([a,b])$. Тогда
    \[\oint\limits_{\gamma}Q\ dy=\iint\limits_{\overline{G}}Q'_x\ dxdy\]
    где
    \[\overline{G}=\{(x,y): y\in[a,b],\ \phi_1(y)\leq x\leq \phi_2(y)\}\]
\end{lemma}
\begin{theorem}[Формула Грина для областей специального вида] $\\$
    Пусть $\gamma$ --- кривая, ограничивающая выпуклую область $G$ в $\R^2,\ P,Q\in \mathcal{C}^1(\overline{\Omega})$. Тогда 
    \[\oint\limits_{\gamma}P\ dx+Q\ dy=\iint\limits_{\overline{G}}\left(Q'_x-P'_y\right) dxdy\]
\end{theorem}
\begin{proof}\tab
    \begin{enumerate}
        \item Проведем семейство вертикальных прямых и разрежем область $G$ на области, подходящие под лемму для $P$. Применим лемму для каждой области и сложим интегралы по аддитивности. 
        \item Проведем семейство горизонтальных прямых и разрежем область $G$ на области, подходящие под лемму для $Q$. Применим лемму для каждой области и сложим интегралы по аддитивности.
    \end{enumerate}
    Таким образом, доказав утверждение для $P$ и для $Q$ отдельно, получим утверждение теоремы.
\end{proof}
\begin{theorem}[Достаточное условие потенциального поля в односвязной области]
    Пусть $F=(P,Q)$ --- векторное поле, $\Omega\subset \R^2$ --- односвязная выпуклая область и
    $\partial \Omega$ --- связное множество. Тогда если $P,Q\in \mathcal{C}^1(\overline{\Omega})$ и $Q'_x=P'_y$, то\\
...
\begin{example} 
    ...
    \[P'_y=\frac{y^2-x^2}{(x^2+y^2)^2},\ \ Q'_x=\frac{y^2-x^2}{(x^2+y^2)^2}\ \Rightarrow\ P'_y=Q'_x\]
...
\end{example}
\begin{comm}
    Если пользоваться формулой Грина для треугольника, можно показать это утверждение для произвольных односвязных областей.
\end{comm}
