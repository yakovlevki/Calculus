...
\begin{definition}
    ...
    \begin{enumerate}
        \item Если $\Pi$ --- брус, то определим $\mu(\Pi)=V(\Pi)$ и если $\Pi_i$ --- брусы, то по определению положим 
...
\end{definition}
\begin{theorem} [Критерий измеримости по Жордану в терминах кратного интеграла] $\\$
...
\begin{proof}\tab
    \begin{itemize}
        \item[$(\Rightarrow)$:] Пусть $\mu_*(\Phi)=\mu^*(\Phi)=\mu(\Phi)$. Тогда $\forall \epsilon>0\ \exists\ P_{\epsilon},\ Q_{\epsilon},\ Q_{\epsilon}\subset \Phi\subset P_{\epsilon}$:
        ...
        \[\overline{\overline{s}}(\chi_{\Phi},T)=\sum\limits_{p_i}\inf\limits_{x\in \Delta p}\chi_{\Phi}(x)|\Delta p|,\ \ \underline{\underline{S}}(\chi_{\Phi},T)=\sum\limits_{p}\sup\limits_{x\in \Delta p}\chi_{\Phi}(x)|\Delta p|\]
        ...
        \end{enumerate}
        Определим $Q_{\epsilon}$ как объединение брусов из первой группы, а $P_{\epsilon}$ как объединение брусов из первой и второй группы. Заметим, что $\overline{\overline{s}}(\chi_{\Phi}, T)$ является объединением брусов из первой группы, а $\underline{\underline{S}}(\chi_{\Phi}, T)$ --- объединением ячеек из первой и второй группы. Значит
...
\begin{proof} Докажем по схеме $(1)\Rightarrow(2) \Rightarrow(3) \Rightarrow (1)$
    \begin{enumerate}
        ...
        Во второй части доказательства предыдщуей теоремы было показано, что
        \[\mu(P_{\epsilon})-\mu(Q_{\epsilon})<\epsilon\]
        \item $(2) \Rightarrow (3)$:\\
        Пусть 
        \[\mu(P_{\epsilon})-\mu(Q_{\epsilon})<\epsilon\]
