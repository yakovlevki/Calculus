...
где $|\Delta_j|$ - длина отрезка $\Delta_j$. Тогда интегралом Римана назывался предел
\[I=\lim\limits_{d(T)\to 0}\sigma(f,T,H)\]
Также интеграл Римана можно ввести через суммы Дарбу.
Назовем верхней и нижней суммой Дарбу соответственно выражения:
...
\subsection{Кратный интеграл Римана по брусу}
\begin{definition}
    Замкнутым брусом в $\R^k$ называется декартово произведение
    \[\Pi=[a_1,b_1]\times [a_2,b_2]\times \dots\times [a_k, b_k]\]
\end{definition}
\begin{definition}
    Пусть дан брус 
    $\Pi=[a_1,b_1]\times [a_2,b_2]\times \dots\times [a_k, b_k]$, а $T_1,\dots,T_k$ являются разбиениями $[a_1,b_1], \dots, [a_k,b_k]$ соответственно. Тогда 
    \[T=T_1\times\dots\times T_k\] 
    называется разбиением бруса $\Pi$. Брус из разбиения $T=T_1\times \dots\times T_k$ будем обозначать $\Delta_{j_1,\dots,j_k}$, а его объем $|\Delta_{j_1,\dots,j_k}|$. Разметкой разбиения также назовем множество $H=\{\xi_{j_1,\dots,j_k}\}_{j_1,\dots,j_k=1}^n$, где $\xi_{j_1,\dots,j_k}\in \Delta_{j_1,\dots,j_k}$.
\end{definition}
\begin{definition}
...
\begin{enumerate}
    \item $V(\Phi)\geq 0$
    \item $\Phi_1=\Phi_2 \Rightarrow V(\Phi_1)=V(\Phi_2)$
    \item $V(\Phi)=V(\Phi_1)+V(\Phi_2)$, если $\Phi=\Phi_1\sqcup\Phi_2$
    \item Объем единичного куба равен $1$.
\end{enumerate}
таким образом, объемом бруса $\Pi$ называется
\[V(\Pi)=(b_1-a_1)\dots(b_k-a_k)\]
\end{definition}
\begin{definition}
    Диаметром бруса $\Pi$ назовем
    \[\text{diam}(\Pi)=\sqrt{(b_1-a_1)^2+\dots+(b_k-a_k)^2}\]
Тогда диаметром $d(T)$ разбиения $T=\{\Delta_j\}$ называется наибольший из диаметров $\Delta_j$.
\end{definition}
\begin{definition}
    $I$ называется кратным интегралом функции $f$ по брусу $\Pi$, если
    \[\forall \epsilon>0\ \exists\ \delta>0,\ \forall T: d(T)<\delta\ \forall H: |\sigma(f,T,H)-I|<\epsilon\]
\end{definition}
\subsection{Суммы Дарбу для кратного интеграла}
Далее пусть дан брус $\Pi$ и его разбиение $T=\{\Delta_j\}$.
\begin{definition}
    Назовем верхней и нижней суммой Дарбу соответственно выражения:
...
\end{definition}
\begin{definition}
    Разбиение $T'=T'_1\times\dots\times T'_k$  называется измельчением разбиения $T=T_1\times\dots\times T_k$, если $\forall i=1,\dots,k:\ T'_i$ является измельчением $T_i$. 
\end{definition}
...
\end{statement}
\begin{proof}
    Cравним $\overline{\overline{s}}(f, T)$ и $\overline{\overline{s}}(f, T')$, где $T'$ получена добавлением разреза. После добавления, брус $\Delta_j$ разбился на два новых бруса $\Delta_j'$ и $\Delta_j''$, обозначим: $m_j', m_j''$ - инфинумы $f$ на $\Delta_j'$ и $\Delta_j''$ соответсвенно. Тогда 
    \begin{multline*}
        \overline{\overline{s}}(f,T')-\overline{\overline{s}}(f,T)\overset{(1)}{=}\sum\limits_{j}(m_j'\cdot |\Delta_j'|+m_j''\cdot |\Delta_j''|-m_j(|\Delta_j'|+|\Delta_j''|))=\\
...
\end{proof}
\begin{statement} \label{razrezy}
        Пусть к разбиению $T$ добавили $p$ разрезов (к разбиениям $T_1,\dots,T_k)$. Тогда 
        ...
        где $M=\sup\limits_{x\in R}f(x),\ m=\inf\limits_{x\in R}f(x),\ d$ --- диаметр $\Pi,\ \delta=d(T)$ - диаметр $T$.
\end{statement}
\begin{proof} Докажем для нижней суммы. Пусть $T'$ получено из $T$ добавлением одного разреза. Тогда
    \begin{multline*}
        \overline{\overline{s}}(f,T')-\overline{\overline{s}}(f,T)\overset{(1)}{=}\sum\limits_{j}((m_j'-m_j)\cdot |\Delta_j'|+(m''_j-m_j)\cdot |\Delta_j''|)\overset{(2)}{\leq}\\
        \overset{(2)}{\leq} \sum\limits_{j} ((M-m)\cdot |\Delta_j'|+(M-m)\cdot |\Delta_j''|)=(M-m)\sum\limits_{j}(|\Delta_j'|+|\Delta_j''|)=\\
        =(M-m)\cdot |\Pi_1|\overset{(3)}{\leq} (M-m)\cdot d^{k-1}\cdot \delta
    \end{multline*}
    где $|\Pi_1|$ --- суммарный объём тех брусов, которые были разрезаны. Проделав эту операцию $p$ раз, получим искомое утверждение.\\
    (1): Суммируем только по тем брусам, по которым прошел разрез\\
    (2): $m_j', m_j''\leq M,\ m_j\geq m$.\\
    (3): Одна сторона, которая направлена вдоль разреза, не длиннее $\delta$, а остальные $k-1$ стороны не длиннее диаметра $d$ бруса $\Pi$. Таким образом $|\Pi_1|\leq d^{k-1}\cdot \delta$. \\
    Аналогично можно показать для верхней суммы Дарбу.
\end{proof}
\begin{statement}
...
\begin{proof}
    ...
    \[\forall \epsilon>0\ \exists\ \xi_j\in \Delta_j: 0\leq f(\xi_j)-m_j<\frac{\epsilon}{|\Pi|}\]
    отсюда
    \[0\leq \sigma(f,T,H)-\overline{\overline{s}}(f,T)=\sum\limits_{j}(f(\xi_j)-m_j)\cdot |\Delta_j|<\sum\limits_{j}\frac{\epsilon}{|\Pi|}\cdot |\Delta_j|=\frac{\epsilon}{|\Pi|}\cdot \sum\limits_{j}|\Delta_j|=\epsilon\]
    а это в точности означает, что
    \[\overline{\overline{s}}(f,T)=\inf\limits_H\sigma(f,T,H)\]
    Аналогично для верхней суммы Дарбу.
\end{proof}
\begin{statement}
    Пусть $T'$ и $T''$ --- любые разбиения бруса $\Pi$. Тогда 
...
\begin{proof}
    ...
    \[\overline{\overline{s}}(f,T')\leq \overline{\overline{s}}(f,T)\leq \underline{\underline{S}}(f,T)\leq \underline{\underline{S}}(f,T'')\]
\end{proof}
\begin{definition}
...
\begin{theorem}[Необходимое условие интегрируемости по Риману на брусе]
    $\\$Если существует интеграл $I$ от $f$ на $\Pi$, то $f$ ограничена на $\Pi$, то есть
...
\begin{proof}
    ...
    значит, $f$ ограничена на каждом $\Delta_{j_0} \Rightarrow f$ - ограничена на $\Pi$.
\end{proof}
\begin{example}[Необходимое условие не является достаточным]
    ...
    \[\underline{\underline{S}}=\sum\limits_{j}M_j\cdot |\Delta_j|=\sum\limits_{j}1\cdot |\Delta_j|=|\Pi|\]
    Итак
    \[\forall I\in \R\ \ \exists\ \epsilon=\frac{|\Pi|}{3}>0,\ \forall \delta>0\ \ \exists\ T,H,\ d(T)<\delta: |\sigma(f,T,H)-I|\geq \epsilon\]
