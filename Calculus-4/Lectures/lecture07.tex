...
\begin{example}
    ...
    \[\mu(E \times [0, 1]) = 0 \cdot 1 = 0\ \Rightarrow\ \exists \iint\limits_{[0,1]^2} f(x, y)\ dxdy = 0\]
    однако $\forall x \in E: f(x, y) \equiv \mathcal{D}(y)$ --- неинтегрируема на отрезке $[0, 1]$. Значит,
    \[\exists \iint\limits_{[0,1]^2} f(x, y)\ dxdy, \text{  но   } \nexists \int\limits_{0}^{1}\left(\int\limits_{0}^{1} f(x, y) \ dy\right) dx\]
\end{example}
\begin{example}
    Рассмотрим 
    \[f(x,y) = \frac{x^2 - y^2}{(x^2 + y^2)^2}\] 
    на открытом брусе $(0, 1)^2$. Кратный интеграл не существует в силу неограниченности в окрестности точки $(0, 0)$, однако
    \[\int\limits_{0}^{1}\left(\int\limits_{0}^{1} \frac{x^2 - y^2}{(x^2 + y^2)^2} \ dx\right) dy = \int\limits_{0}^{1}\left(\frac{-x}{x^2 + y^2}\right)\Bigg|_0^1\ dy = -\int\limits_{0}^{1}\frac{1}{1 + y^2}\ dy = -\frac{\pi}{4};\]
    \[\int\limits_{0}^{1}\left(\int\limits_{0}^{1} \frac{x^2 - y^2}{(x^2 + y^2)^2} \ dy\right) dx = \int\limits_{0}^{1}\left(\frac{y}{x^2 + y^2}\right)\Bigg|_0^1\ dx = \int\limits_{0}^{1}\frac{1}{1 + x^2}\ dx = \frac{\pi}{4}\]
...
\begin{proof}
    ...
    \begin{multline*}
        \sigma\left(F , T_1, H_1\right)=\\
        =\sum\limits_{i}\left( \left(\idotsint\limits_{\Pi_2} f(\xi_i, x_{m+1}, \dots, x_k)\ dx_{m+1}\dots dx_{k}\right)\cdot \mu(\Delta_i^1)\right)=\\
        =\sum\limits_{i} F(\xi_i) \mu(\Delta_i^1)
    \end{multline*}
    Обозначим
    \[M_{ij}=\sup\limits_{\Delta_{ij}} f,\ m_{ij}=\inf\limits_{\Delta_{ij}} f\]
    Тогда
    \[m_{ij}\cdot \mu(\Delta_j^2)\leq\idotsint\limits_{\Delta_j^2} f(\xi_i, x_{m+1},\dots,x_k)\ dx_{m+1}\dots dx_k\leq M_{ij}\cdot \mu(\Delta_j^2)\]
    и выражение
    \begin{multline*}
        F(\xi_i)=\idotsint\limits_{\Pi_2} f(\xi_i, x_{m+1}, \dots, x_k)\ dx_{m+1}\dots dx_k=\\
        =\sum\limits_{j} \idotsint\limits_{\Delta_j^2} f(\xi_i, x_{m+1},\dots,x_k)\ dx_{m+1}\dots dx_k
    \end{multline*}
    оценивается как
...
\end{consequense}
\begin{consequense}(Принцип Кавальери для объёмов)\\
    Пусть даны два тела $\Phi_1, \Phi_2 \subset \R^3$ такие, что при любом сечении плоскостью, параллельной данной, площади полученных сечений равны. Тогда объёмы $\Phi_1$ и $\Phi_2$ равны.
\end{consequense}
\begin{proof}
    Рассмотрим ось $x_1$, пусть $\Omega_1(x_1)$ и $\Omega_2(x_1)$ --- плоские сечения тел $\Phi_1$ и $\Phi_2$ соответственно. Объемы тел равны
    \[V(\Phi_1)=\iiint\limits_{\Phi_1}1\ dx_1 dx_2 dx_3, \ \ V(\Phi_2)=\iiint\limits_{\Phi_2}1\ dx_1 dx_2 dx_3\]
    Применим второе следствие при $f\equiv 1$:
    \[V(\Phi_1)=\int\limits_{a}^{b}\left(\ \ \iint\limits_{\Omega_1(x_1)}1\ dx_2 dx_3\right)dx_1=\int\limits_{a}^{b}S(\Omega_1(x_1))\ dx_1\]
    \[V(\Phi_2)=\int\limits_{a}^{b}\left(\ \ \iint\limits_{\Omega_2(x_1)}1\ dx_2 dx_3\right)dx_1=\int\limits_{a}^{b}S(\Omega_2(x_1))\ dx_1\]
    По условию $S(\Omega_1(x_1))=S(\Omega_2(x_1))$, значит $V(\Phi_1)=V(\Phi_1)$.
\end{proof}
\begin{example}
