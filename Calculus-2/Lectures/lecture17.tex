...
\end{proof}
\begin{definition}\
    Градиентом функции $f$ называется ковектор
    \[
...
\begin{definition}
    ...
    называется вторым дифференциалом функции $f$. Если $f\in \mathcal{C}^k$, то последовательно определяются дифференциалы до $k$ порядка.
\end{definition} 
...
\end{theorem} 
\begin{proof}
    Пусть $X_1=x_{01}+(x_1-x_{01})t, \dots, X_n=x_{0n}+(x_n-x_{0n})t,\\ t\in [0,1]$
    \[F(t)=f(X_1(t),\dots, X_n(t))\]
    \begin{multline*}
        f(\bar{x})=F(1)=\sum\limits_{m=0}^{k-1}\frac{1}{m!}\cdot F^{(m)}(0)\cdot (1-0)+\frac{1}{k!}\cdot F^{(k)}(\theta)(1-0)=\\\
        =\sum\limits_{m=0}^{k-1}\frac{1}{m!}\sum\limits_{i_1,\dots,i_m=1}^{n}\frac{\partial^m {f}}{\partial {x_{i_1}} \dots \partial x_{i_m}}\cdot (x_{i_1}-x_{0i_1})\dots (x_{i_m}-x_{0i_m})+\\
        +\frac{1}{k!}\sum\limits_{i_1,\dots,i_k=1}^{n}\frac{\partial^k {f}}{\partial {x_{i_1}}\dots \partial x_{i_k}}(\bar{x}_0+\theta(\bar{x}-\bar{x}_0)) (x_{i_1}-x_{0i_1})\dots (x_{i_k}-x_{0i_k})
    \end{multline*}
\end{proof} 