...
\begin{examples}\tab
        ...
        Если
        \[\ln{n}\cdot(n\left(\frac{a_n}{a_{n+1}}\right)-1)\geq 1+\alpha\]
        то ряд $(*)$ сходится. Если
        \[\ln{n}\cdot(n\left(\frac{a_n}{a_{n+1}}\right)-1)\leq 1\]
        то ряд $(*)$ расходится. Запишем в предельной форме:\\
        Если 
        \[\exists\ \lim\limits_{n\to\infty}(\ln{n}\cdot(n\left(\frac{a_n}{a_{n+1}}\right)-1))=q>1\]
        то ряд ($*$) сходится. Если
        \[\exists\ \lim\limits_{n\to\infty}(\ln{n}\cdot(n\left(\frac{a_n}{a_{n+1}}\right)-1))=q<1\]
        то ряд ($*$) расходится.
