...
\begin{theorem} (Почленное интегрирование)\\
    ...
    \[\int\limits_{a}^{x}f_n(t)\ dt\ \overset{[a,b]}{\rightrightarrows} \int\limits_{a}^{x}f(t)\ dt\]
\end{theorem}
\begin{proof}
...
\begin{theorem} (Почленное дифференцирование)\\
    Пусть $\{f_n(x)\}_{n=1}^{\infty}$ такая, что $f_n(x)\in \mathcal{C}^1[a,b],\ \exists\ x_0\in [a,b]: f_n(x_0)\to \alpha$.\vspace{0.2cm}\\
    Пусть $f_n'(x)\overset{[a,b]}\rightrightarrows g(x)$. Тогда: 
    \[f_n(x)\overset{[a,b]}\rightrightarrows f(x),\ f(x)\in \mathcal{C}^1[a,b] \text{ и }
    f'(x)=g(x)\]
\end{theorem}
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item Если $S_n(x)\overset{A}{\rightrightarrows} S(x)$, то говорят, что ряд $(1)$ сходится равномерно на $A$. 
...
\begin{theorem} (Необходимое условие равномерной сходимости)\\
    \[\sum_{n=1}^{\infty}a_n(x)\overset{A}{\rightrightarrows}S(x)\]
    Тогда $|a_n(x)|\overset{A}{\rightrightarrows}0$.
\end{theorem}
\begin{proof}
...
\end{theorem}
\begin{proof}
    $S_n(x)\overset{[a,b]}{\rightrightarrows} S(x)$. Проинтегрируем почленно:
    \[\int\limits_{a}^{x}S_n(t)\ dt \overset{[a,b]}{\rightrightarrows} \int\limits_{a}^{x} S(t)\ dt\]
    что можно $\forall x\in [a,b]$ записать, как:
    \[\lim\limits_{k\to\infty}\ \int\limits_{a}^{x} \left(\sum\limits_{n=1}^{k} a_n(t)\right)dt=\int\limits_{a}^{x}\left(\sum\limits_{n=1}^{\infty}a_n(t)\right)dt\]
    Таким образом
    \begin{multline*}
...
\begin{theorem} (Признак Вейерштрасса равномерной сходимости)\\
    Пусть $\forall n: |a_n(x)|\leq \alpha_n,\ \forall x\in A$. Если ряд
...
\begin{proof} Аналогично как для числовых рядов. Введем:
    ...
    \end{multline*}
        \begin{itemize}
            \item[($\mathcal{A}$):] 
            \[\left|\sum_{n=k}^{m-1}A_n(x)(b_n(x)-b_{n+1}(x))+A_m(x) b_m(x)\right|\leq \epsilon\cdot 3M\]
            \item[($\mathcal{D}$):] 
            \[\left|\sum_{n=k}^{m-1}A_n(x)(b_n(x)-b_{n+1}(x))+A_m(x) b_m(x)\right|\leq 2M\cdot 3\epsilon\]
        \end{itemize}
\end{proof}