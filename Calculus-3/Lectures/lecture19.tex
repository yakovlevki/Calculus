...
\begin{proof}
    ...
    \end{multline*}
    % там по равномерной непрерывности разность оценивается
\end{proof}
...
\end{theorem}
\begin{proof}
    Делаем такое же преобразование, как в минимальном свойстве коэффициентов Фурье:
    \begin{multline*}
        \frac{1}{\pi}\int\limits_{-\pi}^{\pi}(f(x)-T_N(x))^2\ dx=\frac{1}{\pi}\int\limits_{-\pi}^{\pi}f^2(x)\ dx+\\
        +2\left(\frac{a_0}{2}-\frac{A_0}{2}\right)^2+\sum\limits_{n=1}^{N}((a_n-A_n)^2+(b_n-B_n)^2)-\frac{a_0^2}{2}-\sum\limits_{n=1}^{N}(a_n^2+b_n^2)
    \end{multline*}
