...
\end{consequense}
\begin{proof}
    По неравенству Бесселя частичные суммы ряда
    \[\sum\limits_{n=1}^{\infty}(a_n^2+b_n^2)\]
    ограничены, при этои ряд стостоит из неотрицательных членов, значит он сходится.
    Таким образом, по необходимому условию сходимости
    \[a_n\to 0,\ \ b_n\to 0\]
\end{proof}
\subsection{Ядра Дирихле} % (fold)
...
\begin{proof} \tab
    \begin{enumerate} 
        \item Очев
        \item 
        \[D_N(0)=\frac{1}{2}+\sum\limits_{n=1}^{N}\cos{0}=N+\frac{1}{2}\]
...
\end{proof}
\begin{comm} 
    Частичную сумму ряда Фурье можно представить через ядра Дирихле:
    \begin{multline*}
        F_N(x)=\frac{a_0}{2}+\sum\limits_{n=1}^{N}(a_n\cdot \cos{(nx)}+b_n\cdot \sin{(nx)})=\\
