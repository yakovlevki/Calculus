...
\begin{theorem}
    ...
    Тогда
    \[\sum_{n=0}^{\infty}\frac{f^{(n)}(x_0)}{n!}\cdot x^n = f(x) \text{ на } (-a, a)\]
\end{theorem}
\begin{proof}
...
\begin{proof}
    ...
    % \end{multline*}
    Оценим разность:
    \[\left|\frac{1}{N}\sum_{n=1}^{N}a_n - A\right| = \frac{1}{N}\left|\sum\limits_{n=1}^{N}a_n - AN\right| = \frac{1}{N}\left|\sum\limits_{n=1}^{N}(a_n - A)\right|\]
    Фиксируем некоторое число $N_1 < N$ и разобьём сумму на две части:
    \[\frac{1}{N}\sum\limits_{n=1}^{N}|a_n - A| = \frac{1}{N}\sum\limits_{n=1}^{N_1}|a_n - A| + \frac{1}{N}\sum\limits_{n=N_1 + 1}^{N}|a_n - A|\]
    Так как $a_n \to A$, $ \forall \epsilon > 0 \ \exists N_1 : \ |a_n - A| < \epsilon$, тогда: 
...
\begin{proof}
    ...
    \begin{multline*}
        \left|\sum_{n=0}^{N_x}a_n(1-x^n)\right| = \left|\sum_{n=0}^{N_x}a_n(1-x)(1+x+x^2+...+x^{n-1})\right| \leq \\
