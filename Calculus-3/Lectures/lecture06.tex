...
\begin{definition}
    ...
    называются степенными рядами. $a_n$ - коэффициенты степенного ряда, $x_0$ - центр разложения.
\end{definition}
\begin{comm}
...
\begin{proof}\tab
    \begin{enumerate}
        \item Пусть $x: |x|<|x_1|$.
        \[|a_n x^n|=|a_n x_1^n|\cdot \left|\frac{x}{x_1}\right|^n \overset{(1)}{\leq} M\cdot \left|\frac{x}{x_1}\right|^n\]
...
\begin{comm}
    ...
    Тогда из первой теоремы Абеля следует, что $A=\{0\}$ или $\exists\ a>0: (-a,a)\subset A$, а также $\exists\ b>0: (-\infty, -b)\cup(b,+\infty)\subset B$.
\end{comm}
\begin{statement}
    $a=b$
\end{statement}
