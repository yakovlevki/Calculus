\begin{comm}Из неравенства
    ...
    следует, что если сущесвует интерал от квадрата, то существует и от модуля.
\end{comm}
\begin{theorem} (Принцип локализации Римана)\\
    Пусть существует интегралы
    ...
    равносильно тому, что $\forall \delta>0$ сущесвтует 
...
\begin{proof}
    ...
    \begin{multline*}
        \lim\limits_{N\to\infty}\frac{1}{\pi}\cdot \int\limits_{\delta}^{\pi}\cdot\frac{f(x_0+t)+f(x_0-t)}{2}\cdot \cos{(Nt)}\ dt+\\
        +\frac{1}{\pi}\cdot \frac{f(x_0+t)+f(x_0-t)}{2}\cdot \cot{\frac{t}{2}}\sin{(Nt)}\ dt
    \end{multline*}
    Пусть
    \[h(t)=\begin{cases}
        ...
        0,\ t\in [-\pi, \delta]
    \end{cases}\]
    тогда $I_1=a_N(h)\to 0,\ N\to \infty$. Теперь пусть
    \[g(t)=\begin{cases}
        \frac{f(x_0+t)+f(x_0-t)}{2}\cdot \cot{\frac{t}{2}},\ t\in [\delta, \pi]\\
        0,\ t\in [-\pi,\delta)
    \end{cases}\]
    тогда $I_2=b_N(g)\to 0,\ N\to \infty$.
\end{proof}
