...
\end{theorem}
\begin{proof}
    Из условий теоремы существует интерал
    \[\int\limits_{-\pi}^{\pi}(f'(x))^2\ dx,\ f'(x)\sim \frac{a_0}{2}+\sum\limits_{n=1}^{\infty}(\alpha_n\cdot \cos{(nx)}+\beta_n\cdot \sin{(nx)})\]
    \[a_n=-\frac{\beta_n}{n},\ b_n=\frac{\alpha_n}{n}\]
    и, поскольку
    \[\frac{|\alpha_n|}{n}\leq \alpha_n^2+\frac{1}{n^2},\ \frac{|\beta_n|}{n}\leq \beta_n^2+\frac{1}{n^2}\]
...
\begin{comm} (Про почленное интегрирование)\\
    ...
    \[\int\limits_{0}^{x}\left(f(x)-\frac{a_0}{2}\right) dx=\sum\limits_{n=1}^{\infty}\left(-\frac{b_n}{n}\cdot \cos{(nx)}+\frac{a_n}{n}\cdot \sin{(nx)}\right)+\sum\limits_{n=1}^{\infty}\frac{b_n}{n}\]
    \[\frac{|\alpha_n|}{n}\leq \alpha_n^2+\frac{1}{n^2},\ \frac{|\beta_n|}{n}\leq \beta_n^2+\frac{1}{n^2}\]
\end{comm}
\begin{examples}\tab % говорил что обязательные для экзамена примеры
    \begin{enumerate}
        ...
        \item Возьмем $f(x)=|x|$ на $[-\pi,\pi]$ и периодически продолжим.
            ...
            Отюда, поскольку ряд сходится равномерно от непрерывной функции к непрерывной функции, то можно подставить 0 и получить
...
\begin{lemma} (Свойства ядер Фейера)
    \begin{enumerate}
        ...
        \item
        \[\frac{1}{\pi}\int\limits_{-\pi}^{\pi}\Psi(x)\ dx=1\]
        \item $\forall \delta>0: $
    ...
    \end{enumerate}
\end{lemma}
\begin{proof} Докажем 4.
    \[0\leq \int\limits_{\delta}^{\pi}\frac{\sin^2(\frac{N+1}{2}\cdot x)}{2(N+1)\cdot \sin^2{\frac{x}{2}}}\ dx\leq \frac{1}{2(N+1)}\cdot \int\limits_{\delta}^{\pi}\frac{dx}{\sin^2{\frac{x}{2}}}\leq \frac{C}{2(N+1)}\]
\end{proof}