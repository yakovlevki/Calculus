\begin{theorem} (Достаточное условие равномерной сходимости ряда Фурье)\\
...
\begin{proof}
    ...
    \[\alpha_n = \int\limits_{-\pi}^{\pi}f'(x)\cos(nx)dx = \int\limits_{-\pi}^{\pi}\cos{(nx)}\ d(f(x)) = n\int\limits_{-\pi}^{\pi}f(x)\sin(nx)\ dx = nb_n\]
    Отсюда (и аналогично для $\beta_n$) имеем, что
    \[a_n=-\frac{\beta_n}{n},\ b_n=\frac{\alpha_n}{n}\]
    По неравенству
    \[|ab| \leq \frac{a^2 + b^2}{2}\]
    получим, что
    \[|a_n| = \frac{|\beta_n|}{n}\leq \frac{1}{2}\left(\alpha_n^2+\frac{1}{n^2}\right),\ \ |b_n| = \frac{|\alpha_n|}{n}\leq \frac{1}{2}\left(\beta_n^2+\frac{1}{n^2}\right)\]
...
\begin{examples}\tab % говорил что обязательные для экзамена примеры
    \begin{enumerate}
        ...
        \begin{multline*}
            ...
            =-\frac{2}{\pi n}\cdot \frac{\pi-x}{2}\cdot \cos{(nx)} \Big{|}_0^{\pi}-\frac{1}{\pi n}\int\limits_{0}^{\pi}\cos{(nx)}\ dx=\\
