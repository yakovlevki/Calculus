...
\begin{definition}
    \[(\alpha, \beta)=\int\limits_{0}^{1}x^{\alpha-1}(1-x)^{\beta-1}\ dx,\ \alpha,\ \beta>0\]
\end{definition}
\begin{statement}
...
\begin{proof}
    \begin{multline*}
        ...
        =\frac{\alpha}{\beta}\int\limits_{0}^{1}c^{\alpha-1}(1-x)^{\beta}\ dx=\frac{\alpha}{\beta}\int\limits_{0}^{1}x^{\alpha-1}(1-x)(1-x)^{\beta-1}\ dx=\\
...
\end{statement}
\begin{proof}
    \[\int\limits_{0}^{1}t^{\beta-1}(1-t)^{\alpha-1}\ dt=\left|t=\frac{1}{1+x}\right|=\int\limits_{0}^{+\infty}\frac{x^{\alpha-1}}{(1+x)^{\beta-1}(1+x)^{\alpha-1}(1+x)^2}\ dx\]
\end{proof}
...
\begin{proof} Сначала рассмотрим $\alpha,\ \beta>1$
    \begin{multline*}
        \Gamma(\alpha+\beta)\cdot B(\alpha, \beta)=\int\limits_{0}^{1}x^{\alpha-1}(1-x)^{\beta-1}\cdot \Gamma(\alpha+\beta) dx=\\
    ...
    Далее, используя реккурентную формулу, распространяем на случай\\
    $\alpha, \beta>0$.
\end{proof}
\begin{comm}
    Интегралы вида
    \[\int\limits_{0}^{\frac{\pi}{2}}\sin^{\theta}x\cdot \cos^{\kappa}x\ dx\]
    сводятся к бета-функции.
\end{comm}
\newpage
\section{Ряды Фурье}
...
\begin{statement} (Свойства тригонометрической системы)\\
    \[\int\limits_{0}^{\pi}1\ dx=\pi\]
    \[dx=\pi,\ \forall n\geq 1\ \int\limits_{0}^{\pi}\sin^2{(nx)}\ dx=\int\limits_{0}^{\pi}\cos^2{(nx)}\ dx=\frac{\pi}{2}\]
    \[\forall m,n\geq 1,\ m\neq n\ \int\limits_{0}^{\pi}\sin{(nx)}\cdot \sin{(mx)}\ dx=\int\limits_{0}^{\pi}\cos{(nx)}\cdot \cos{(mx)}\ dx=0\]
    \[\forall n\geq 0,m\geq 1\ \int\limits_{0}^{\pi}\cos{(nx)}\cdot \sin{(mx)}\ dx=0\]
\end{statement}
\begin{proof}
    Очев.
\end{proof}
