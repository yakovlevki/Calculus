...
\begin{proof} Сначала рассмотрим $\alpha,\ \beta>1$
    ...
    Далее, используя рекуррентную формулу, распространяем на $\alpha, \beta>0$:
    \begin{multline*}
        B(\alpha, \beta) = \frac{\alpha+\beta}{\alpha}B(\alpha+1, \beta) = \frac{(\alpha+ \beta)
        (\alpha+\beta+1)}{\alpha\beta}B(\alpha+1, \beta + 1) =\\= \frac{(\alpha+ \beta)
        (\alpha+\beta+1)}{\alpha\beta}\cdot \frac{\Gamma(\alpha+1)\Gamma(\beta + 1)}{\Gamma(\alpha + \beta + 2)} = \frac{\Gamma(\alpha) \Gamma(\beta)}{\Gamma(\alpha+\beta)}
    \end{multline*}
\end{proof}
\begin{comm}
    Интегралы вида
...
\end{theorem}
\begin{proof} 
    Покажем для $b_k$. Для каждого $k$ домножим на $\sin{(kx)}$:
    ...
    Поскольку есть равномерная сходимость, то можем почленно интегрировать:
    \[\frac{a_0}{2}\int\limits_{-\pi}^{\pi}\sin{(kx)}\ dx=0\]
    \[a_n \int\limits_{-\pi}^{\pi} \sin{(kx)}\cos{(nx)}\ dx =0\]
    \[b_n \int\limits_{-\pi}^{\pi} \sin{(kx)}\sin{(nx)}\ dx=\begin{cases}
        0,\ k\neq n\\
        \pi,\  k=n
    \end{cases}\]
    Таким образом
    \[\int\limits_{-\pi}^{\pi}f(x)\sin{kx}\ dx=\pi \cdot b_k\]
    Аналогично, домножая на $\cos{(kx)}$, можно получить выражение для $a_k$.
\end{proof}
