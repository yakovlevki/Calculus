...
\begin{multline*}
    \prod\limits_{k=0}^{n-1}(x+k)=\frac{1}{\prod\limits_{k=1}^{n-1}k\cdot x(1+x)(1+\frac{x}{2})\dots(1+\frac{x}{n-1})}=\\
    =\frac{1}{(n-1)!\cdot x(1+x)(1+\frac{x}{2})\dots(1+\frac{x}{n-1})}
\end{multline*}
...
\end{multline*}
Поскольку $(1+\frac{x}{k})^{-1}(1+\frac{1}{k})^x=O(\frac{1}{k^2})$, то существует предел
...
\end{theorem}
\begin{proof}
    \[\lim\limits_{n\to\infty}\frac{\Gamma_n(x+1)}{\Gamma_n(x)}=\lim\limits_{n\to\infty}\ \cfrac{(n-1)!\ n^{x+1}\cdot \prod\limits_{k=0}^{n-1}(x+k)}{\prod\limits_{k=0}^{n-1}(x+k+1) (n-1)!\ n^x}=\lim\limits_{n\to\infty}\frac{nx}{x+n}=x\]
\end{proof}
\begin{theorem}
    $\forall x>0$:
...
\begin{proof}
    \begin{multline*}
        \int\limits_{0}^{1}(1-t)^n\ dt=\frac{1}{x}\cdot \int\limits_{0}^{1}(1-t)^n\ dt^x=\frac{n}{x}\int\limits_{0}^{1}t^x(1-t)^{n-1}\ dt=\dots=\\
    ...
    \end{multline*}
    \[\Gamma_{n+1}(x)=(n+1)^x\cdot \int\limits_{0}^{1}t^{x-1}(1-t)^n\ dt=\left|t=\frac{\tau}{n}\right|=\frac{(n+1)^x}{\tau^x}\cdot \int\limits_{0}^{n}\tau^{x-1}\left(1-\frac{\tau}{n}\right)^n\ d\tau\]
    Поскольку
    ...
    Таким образом, условия теоремы равносильны    
    \[\exists\ \lim\limits_{n\to\infty}\ \int\limits_{0}^{n}t^{x-1}(e^t-(1-\frac{t}{n})^n)\ dt=0\]
    \[e^{\alpha}\geq 1-\alpha \ \Rightarrow\ e^{-\frac{t}{n}}\geq 1-\frac{t}{n}\ \Rightarrow\ e^{-t}\geq \left(1-\frac{t}{n}\right)^n\]
    отсюда
    \[0\leq \int\limits_{0}^{n}t^{x-1}\left(e^{-t}-\left(1-\frac{t}{n}\right)^n\right)\ dt\]
    \[e^{\alpha}\geq 1+\alpha \ \Rightarrow\ e^{\frac{t}{n}}\geq 1+\frac{t}{n}\ \Rightarrow\ e^t\geq \left(1+\frac{t}{n}\right)^n\]
    \begin{multline*}
        \int\limits_{0}^{n}t^{x-1}\left(e^{-t}-\left(1-\frac{t}{n}\right)^n\right)\ dt=\int\limits_{0}^{n}t^{x-1}e^{-t}(1-e^t\left(1-\frac{t}{n}\right)^n)\ dt\leq\\
...
\begin{proof}
    ...
    \[\Gamma(n+1)=(n+1)\cdot \Gamma(n)=(n+1)\cdot n!=(n+1)!\]
\end{proof}
...
\begin{proof}
    ...
    \begin{multline*}
        ...
        =\frac{1}{x}\cdot \prod\limits_{k=1}^{\infty}\left(1+\frac{x}{k}\right)^{-1}\left(1+\frac{1}{k}\right)^x\cdot (-x)\cdot \prod\limits_{k=0}^{\infty}\left(1-\frac{x}{k}\right)^{-1}\left(1+\frac{1}{k}\right)^{-x}=\\
        =\frac{\pi}{\pi x}\prod\limits_{k=1}^{\infty}\left(1-\frac{x^2}{k^2}\right)^{-1}\cdot 1\overset{(1)}{=}\frac{\pi}{\sin{(\pi x)}}
    \end{multline*}
...
\begin{example} (Интеграл Эйлера-Пуассона)\\
    ...
    \[\int\limits_{-\infty}^{+\infty}e^{-x^2}\ dx=2 \int\limits_{0}^{+\infty}e^{-t}\cdot t^{-\frac{1}{2}}\ dt=\Gamma\left(\frac{1}{2}\right)=\sqrt{\pi}\]
\end{example}
