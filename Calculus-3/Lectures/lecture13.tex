...
\begin{proof} Для начала вычислим интеграл по частям:
    ...
    Таким образом, условия теоремы равносильны тому, что разность этих пределов равна нулю:  
    ...
    \begin{multline*}
        ...
        \overset{(1)}{\leq}\int\limits_{0}^{n}t^{x-1}e^{-t}\left(1-\left(1-\frac{t^2}{n}\right)\right)\ dt=\frac{1}{n}\int\limits_{0}^{n}t^{x+1}e^{-t}\ dt\overset{(2)}{\leq} \frac{C}{n}\to 0
    \end{multline*}
    (1): По неравенству Бернулли:
    \[(1+x)^n\geq 1+nx\]
    (2): 
    \[C=\int\limits_{0}^{+\infty}t^{x+1}e^{-t}\ dt\]
\end{proof}
\begin{statement}
    \[\Gamma(n+1)=n!\]
\end{statement}
...
\begin{proof}
    ...
    \begin{multline*}
        ...
        =\frac{1}{x}\cdot \prod\limits_{k=1}^{\infty}\left(1+\frac{x}{k}\right)^{-1}\left(1+\frac{1}{k}\right)^x\cdot (-x)\cdot \frac{1}{x}\cdot \prod\limits_{k=1}^{\infty}\left(1-\frac{x}{k}\right)^{-1}\left(1+\frac{1}{k}\right)^{-x}=\\
...
\begin{theorem} (Интеграл Эйлера-Пуассона)\\
    Заметим, что
    \[\Gamma\left(\frac{1}{2}\right)\cdot \Gamma\left(\frac{1}{2}\right)=\frac{\pi}{\sin{\frac{\pi}{2}}}=\pi\]
    Таким образом, можем посчитать интеграл
    \[\int\limits_{-\infty}^{+\infty}e^{-x^2}\ dx=2 \int\limits_{0}^{+\infty}e^{-t}\cdot \frac{1}{2}\cdot t^{-\frac{1}{2}}\ dt=\Gamma\left(\frac{1}{2}\right)=\sqrt{\pi}\]
\end{theorem}
